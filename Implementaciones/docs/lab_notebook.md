...
### Comparación entre modelos: Haiku 4.5 vs Sonnet 4.6 sobre experimento siete

Se re-corrió el experimento siete con Sonnet 4.6 (mismo corpus, mismo schema, mismo prompt y verificador, sólo cambia el modelo) para responder la pregunta abierta de si los modos de fallo observados con Haiku eran modelo-específicos o diseño-específicos. La paridad del verificador estructural sobre experimento dos quedó confirmada en noventa y nueve sobre noventa y nueve antes de la corrida, garantizando que la nueva clase path_step_incoherent no introduce regresiones sobre IR puramente relacional.

El resultado expone una asimetría interesante. Con Sonnet, sólo dos de diez adversariales son atrapadas por el verificador, frente a seis de diez con Haiku tras la rebanada ocho. Pero las ocho adversariales que pasan al motor con Sonnet no son alucinaciones: son reformulaciones agresivas. Cuando se pregunta por el manager de la empresa de Alice, Sonnet construye un patrón Person-works_at-Company-works_at-Person y llama "manager" al coworker; cuando se pregunta por el salario, expone NULL AS salary en COLUMNS sin inventar la columna; cuando se pregunta por hate, construye un path con knows e interpreta la pregunta como "find pairs"; cuando se pregunta por ratings, devuelve nombre e industry en lugar de rating. Ninguno de esos casos referencia un nombre que no exista en el esquema, así que el verificador no tiene material sobre el cual actuar.

Una adversarial donde Sonnet superó claramente a Haiku es adv-08, la pregunta multihop que motivó la rebanada ocho. Sonnet construyó un PathPattern coherente alternando direcciones (Alice-[works_at]->Company<-[works_at]-Bob-[lives_in]->City) que el verificador acepta correctamente, en contraste con la cadena incoherente Company-[lives_in]->City que Haiku había producido y que la rebanada ocho atrapa.

Sonnet también expuso un modo de fallo nuevo que ningún experimento anterior había mostrado. En el control ctrl-05 (find pairs who know each other and work at the same company), Sonnet emitió un MatchPattern con dos sub-patrones donde el segundo reutiliza variables ya bindeadas sin re-declarar sus labels, una construcción válida en el estándar PGQ pero que DuckPGQ rechaza con la restricción "All patterns must bind to a label". Es una limitación operativa del motor, no del lenguaje, análoga a la restricción de variable obligatoria en aristas que ya estaba documentada. El verificador no la atrapa porque acepta vertex patterns sin label.

Lectura para la tesis. La comparación valida una propiedad esperable del enfoque: el verificador provee soundness sobre nombres del esquema, que es exactamente su contrato declarado en el cap. 4. Cuando el modelo es lo suficientemente capaz para no inventar nombres, las alucinaciones de nombre se vuelven raras y la utilidad del verificador disminuye en frecuencia, no en valor. Los modos de fallo que aparecen con modelos más capaces son distintos: drift semántico donde el modelo usa el label equivocado pero existente, y restricciones del motor que la IR estándar no captura. El cap. 4 ya prevé esa transición declarando el régimen de verificación dinámica, donde el motor de ejecución actúa como oráculo parcial y los errores semánticos o ejecutables se detectan por ejecución; la rebanada que materializa ese régimen queda como trabajo futuro pero el experimento de comparación lo motiva con datos. La medición también desafía el corpus adversarial actual: para que el verificador estructural luzca con modelos del nivel de Sonnet, las preguntas tienen que diseñarse con casos donde la pregunta no admite reformulación válida, lo que es un problema de diseño de corpus más sofisticado que la versión actual.
