...
En base a esta complejidad y vacio identificado es que se desarrolló una taxonomía con seis dimensiones ortogonales para clasificar los sistemas \emph{text-to-query}. La taxonomía presentada fue elaborada recogiendo y reorganizando ejes ya presentes en surveys recientes \cite{hong2024llmtext2sql,liu2024nl2sqlsurvey,shi2025surveyllmtext2sql,qin2022recentadvances},
y añadiendo dos contribuciones específicas, producto de la revisión de literatura y de los patrones observados: separar el \emph{paradigma de razonamiento} y las \emph{propiedades evaluativas objetivo} como dimensiones independientes, esto sin descuidar la ortogonalidad entre las dimensiones, de forma tal que un mismo trabajo pueda recibir etiquetas múltiples sin ambigüedad ni superposición entre estas.
...
\begin{enumerate}
    ...
    \item \textbf{Arquitectura del sistema:} La distribución de la ejecución entre invocaciones del modelo y módulos auxiliares, distinguiendo configuraciones monolíticas, \emph{pipelines} modulares, multiagente, \emph{agentic-search} y generación con reordenamiento. Es relevante porque condiciona la inspección de etapas intermedias y la intervención sobre componentes específicos.

    \item \textbf{Representación:} qué estructuras median entre la pregunta y la consulta final, considerando codificación de la entrada, representación intermedia ---ausente, implícita en el esquema, lenguaje dedicado, plan, árbol o descomposición declarativa--- y estrategia de decodificación ---libre, restringida por gramática, composicional o por compilación de la \ac{RI}---. Se considera porque define qué alucinaciones son detectables antes de la consulta final y dónde puede aplicarse verificación estructural.

    \item \textbf{Mecanismos de enriquecimiento contextual:} qué información adicional ancla la salida al esquema y al dominio, mediante \emph{schema linking}, recuperación de contenido de la base de datos, conocimiento externo o ciclos de auto-refinamiento. Se trata como dimensión propia porque su efecto sobre la corrección actúa de forma transversal a la arquitectura y al paradigma de aprendizaje.

    \item \textbf{Paradigma de razonamiento:} cómo se estructura la inferencia que conduce a la consulta, en variantes directa, en cadena de pensamiento, descompositiva, de múltiples caminos o de retroalimentación iterativa. Se separa de la arquitectura porque una misma organización del \emph{pipeline} admite distintos modos de razonamiento estructural y, por lo tanto, distintos perfiles de auditabilidad.

    \item \textbf{Propiedades evaluativas objetivo:} qué propiedades del sistema prioriza explícitamente cada trabajo, entre corrección, eficiencia, robustez, adaptabilidad, escalabilidad e interpretabilidad. Se incorpora como eje independiente para hacer visible el sesgo dominante hacia la corrección y permitir comparaciones sobre propiedades críticas en consultas empresariales.
\end{enumerate}
...
Esta sección analiza la forma en que los sistemas \emph{text-to-query} adaptan su modelo base a la tarea. Aunque la mayoría de los trabajos revisados utiliza \acp{LLM} o modelos preentrenados de tipo \emph{encoder-decoder}, no todos los sistemas los emplean de la misma manera.
...
IRNet~\cite{IRNet} y NatSQL~\cite{gan2021natsql}
...
\begin{enumerate}
    ...
    ---IRNet~\cite{IRNet}, NatSQL~\cite{gan2021natsql},
    ...
    bajo este estándar. STRuCT-LLM~\cite{STRuCT-LLM2025} y
