...
\centering
\footnotesize
\caption{Trabajos representativos en \emph{few-shot in-context learning}, agrupados por mecanismo principal con el que combinan los ejemplos en el \emph{prompt}.}
\label{tab:sota-d1-fewshot}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Mecanismo combinado} & \textbf{Trabajos} \\
\midrule
Descomposición + \emph{prompts} por etapa & DIN-SQL, DEA-SQL, RSL-SQL, TA-SQL \\
Organización multiagente & MAC-SQL, CHESS \\
Cadena de pensamiento (CoT) & DAIL-SQL, ACT-SQL, ODIS, PET-SQL, CoE-SQL, CRED-SQL \\
Selección o reordenamiento de candidatos & PURPLE, Gen-SQL, SGU-SQL \\
Recuperación enfocada de esquema & CRUSH4SQL \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\footnotesize
\caption{Trabajos representativos de \emph{fine-tuning} supervisado, agrupados por la arquitectura base sobre la que aplican el ajuste.}
\label{tab:sota-d1-finetune}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Arquitectura base} & \textbf{Trabajos} \\
\midrule
Redes seq2seq dedicadas & Seq2SQL, BRIDGE, RAT-SQL, SmBoP, RaSaP \\
Codificadores con atención al esquema & RASAT, S$^{2}$SQL, SHiP \\
Variantes de T5 adaptadas & PICARD, Graphix-T5, RESDSQL, T5-3B+NatSQL \\
Generación composicional o por plantilla & SQLFormer, CatSQL \\
Pipeline modular con etapa ajustada & DTS-SQL \\
\bottomrule
\end{tabular}
\end{table}
...
la corrección estructural no descanse exclusivamente en la calidad % LLM-like: maximizador:"exclusivamente"
...
\centering
\footnotesize
\caption{Trabajos representativos en arquitecturas monolíticas, agrupados por época y por la familia de modelos sobre la que operan.}
\label{tab:sota-d2-monoliticas}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Familia / período} & \textbf{Trabajos} \\
\midrule
Seq2seq y codificadores dedicados (pre-LLM) & Seq2SQL, BRIDGE, RAT-SQL, SmBoP, RaSaP, RASAT, S$^{2}$SQL, SHiP \\
Variantes de T5 (pre-LLM) & PICARD, Graphix-T5, T5-3B+NatSQL \\
Generación composicional o por plantilla & SQLFormer, CatSQL \\
\acp{LLM} \emph{in-context} con generación de un solo paso & DAIL-SQL, ACT-SQL, ODIS, SGU-SQL \\
Modelos grandes especializados en SQL o consulta estructurada & CodeS, STRuCT-LLM \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\footnotesize
\caption{Trabajos representativos en \emph{pipelines} modulares, agrupados por el foco principal del pipeline.}
\label{tab:sota-d2-pipelines}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Foco principal del pipeline} & \textbf{Trabajos} \\
\midrule
\emph{Schema linking} dedicado & RESDSQL, DTS-SQL, CRUSH4SQL, TA-SQL \\
Descomposición y refinamiento iterativo & DIN-SQL, DEA-SQL, RSL-SQL, PURPLE \\
Razonamiento con cadena de pensamiento & PET-SQL, SuperSQL, CoE-SQL, CRED-SQL \\
Especialización de dominio o adaptabilidad & FinSQL, ZeroNL2SQL \\
Verificación, selección o composición & CHESS, SelECT-SQL, Gen-SQL, BLENDSQL \\
Plan estructurado sobre el grafo del esquema & SteinerSQL \\
Aprendizaje multi-tarea & TKK \\
Otros (sistemas comerciales o exploratorios) & C3-SQL, Wren AI \\
\bottomrule
\end{tabular}
\end{table}
...
La investigación de esta tesis se posiciona deliberadamente en la % LLM-like: cliché-LLM:"deliberadamente"
...
\centering
\footnotesize
\caption{Trabajos representativos en \emph{schema linking}, agrupados por la forma en que integran la operación en su arquitectura.}
\label{tab:sota-d4-schemalinking}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Forma de integración} & \textbf{Trabajos} \\
\midrule
Atención sensible a relaciones del esquema & RAT-SQL \\
Módulo dedicado pre-generación & RESDSQL, DTS-SQL, CRUSH4SQL, TS-SQL \\
Etapa explícita en pipeline modular & DIN-SQL, DEA-SQL, RSL-SQL, TA-SQL, PURPLE \\
Agente especializado en selección de esquema & CHESS, MAC-SQL \\
\bottomrule
\end{tabular}
\end{table}
...
depende crucialmente de la naturaleza de la retroalimentación: cuando % LLM-like: maximizador:"crucialmente"
...
\centering
\footnotesize
\caption{Trabajos representativos en auto-refinamiento, agrupados por el tipo de retroalimentación que alimenta el ciclo.}
\label{tab:sota-d4-refinement}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Tipo de retroalimentación} & \textbf{Trabajos} \\
\midrule
Auto-corrección sobre texto libre & DIN-SQL, DEA-SQL, RSL-SQL, PURPLE \\
Verificación por agente dedicado & CHESS, MAC-SQL, OpenSearch-SQL \\
Selección entre candidatos refinados & XiYan-SQL, CHASE-SQL \\
Detección o clasificación de errores & ErrorLLM, SQLHD \\
Resolución iterativa de ambigüedad & AmbiSQL \\
\bottomrule
\end{tabular}
\end{table}
...
integración de conocimiento externo de dominio es claramente el menos % LLM-like: maximizador:"claramente"
...
\centering
\footnotesize
\caption{Trabajos representativos en razonamiento directo, agrupados por la familia del modelo de referencia.}
\label{tab:sota-d5-directo}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Familia del modelo} & \textbf{Trabajos} \\
\midrule
Modelos seq2seq dedicados & Seq2SQL, BRIDGE, RAT-SQL, RASAT \\
Variantes de T5 con decodificación restringida o sensible al esquema & PICARD, Graphix-T5, RESDSQL, T5-3B+NatSQL \\
Modelo de código preentrenado y especializado en SQL & CodeS \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\footnotesize
\caption{Trabajos representativos en razonamiento descompositivo, agrupados por la unidad de descomposición que el sistema utiliza.}
\label{tab:sota-d5-descompositivo}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Unidad de descomposición} & \textbf{Trabajos} \\
\midrule
Sub-preguntas a sub-consultas SQL & DIN-SQL, DEA-SQL, RSL-SQL \\
Roles de agentes especializados & MAC-SQL, CHESS \\
Anclaje a tabla previo a la generación & TA-SQL, SuperSQL \\
Plan de ejecución estructurado sobre el grafo del esquema & SteinerSQL \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\footnotesize
\caption{Trabajos representativos en razonamiento de retroalimentación iterativa, agrupados por la fuente que alimenta el ciclo de refinamiento.}
\label{tab:sota-d5-iterativo}
\begin{tabular}{@{}p{0.32\linewidth}p{0.60\linewidth}@{}}
\toprule
\textbf{Fuente del ciclo} & \textbf{Trabajos} \\
\midrule
Agente verificador o \emph{unit tester} & CHESS, MAC-SQL \\
Error de ejecución sobre el motor & OpenSearch-SQL, DIN-SQL, DEA-SQL, RSL-SQL \\
Selección iterativa entre múltiples caminos & CHASE-SQL \\
Modelo dedicado de detección de errores & ErrorLLM, SQLHD \\
Interacción iterativa con el usuario & AmbiSQL \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    variantes.} STRuCT-LLM~\cite{STRuCT-LLM2025} se aproxima al
    razonamiento conjunto sobre tablas y grafos, pero no produce
    \ac{SQL}/\ac{PGQ} estandarizado y verificable.
    Text2GQL-Bench~\cite{text2gqlbench} provee evaluación canónica
    multi-lenguaje sobre Cypher e ISO-GQL, pero opera sobre consultas
    de grafo puras y no contempla la dualidad relacional/grafo propia
    de \ac{SQL}/\ac{PGQ}; aun así, su metodología de construcción de
    \emph{benchmarks} (traducción de esquemas, conversión de datos,
    traducción de consultas y generación jerárquica de preguntas)
    constituye la referencia metodológica más cercana para la
    construcción de un corpus específico del régimen híbrido. Otros
    trabajos como SchemaGraphSQL~\cite{SchemaGraphSQL} se sitúan en
    la frontera, pero sin abordar la generación bajo el estándar
    híbrido.
\end{enumerate}
...
cuyo objetivo es articular estos cuatro componentes en una arquitectura % LLM-like: cliché-LLM:"articular"
coherente que aborde explícitamente la generación confiable de
consultas \ac{SQL}/\ac{PGQ} a partir de lenguaje natural.

\section{Mapa tabular del estado del arte}
\label{sec:sota-tabla}

Las secciones anteriores discutieron los trabajos representativos por dimensión y enunciaron observaciones críticas sobre cada eje. El Cuadro~\ref{tab:sota-mapa} consolida la posición de los trabajos analizados a lo largo de las seis dimensiones del marco taxonómico de la Sección~\ref{sec:sota-taxonomia}, ofreciendo una vista panorámica complementaria a la discusión en prosa. Las filas siguen un orden cronológico y las celdas emplean códigos abreviados; la leyenda completa de códigos figura al pie del cuadro. Cuando un trabajo se posiciona en más de una sub-categoría dentro de una misma dimensión, las etiquetas correspondientes se separan por comas. La marca \texttt{---} indica que la dimensión no fue objeto explícito del trabajo o que la asignación no resultó decidible a partir del material publicado.

\renewcommand{\arraystretch}{1.1}
{\scriptsize
\begin{longtable}{@{}llcccccc@{}}
\caption{Mapa tabular del estado del arte en \emph{text-to-query}. Cada fila clasifica un trabajo en las seis dimensiones del marco taxonómico de la Sección~\ref{sec:sota-taxonomia}.\protect\\
\textbf{D1 Aprendizaje}: F0 zero-shot, FF few-shot, FD few-shot dinámico, TF fine-tuning supervisado, TC preentrenamiento continuo, TM multi-tarea, TR aprendizaje por refuerzo, H híbrido. \textbf{D2 Arquitectura}: M monolítica, PM pipeline modular, MA multiagente, AS agentic-search, RR reranking. \textbf{D3 RI}: --- sin RI explícita, EI esquema como RI implícita, LD lenguaje intermedio dedicado, PE plan estructurado, AB árbol abstracto, DD descomposición declarativa. \textbf{D4 Enriquecimiento}: SL schema linking, CR content retrieval, EK external knowledge, AR auto-refinamiento. \textbf{D5 Razonamiento}: D directo, CoT cadena de pensamiento, Dc descompositivo, MC multi-camino, It iterativo. \textbf{D6 Propiedades}: C corrección, Ef eficiencia, R robustez, Ad adaptabilidad, Es escalabilidad, In interpretabilidad.}
\label{tab:sota-mapa}\\
\toprule
\textbf{Sistema} & \textbf{Año} & \textbf{D1} & \textbf{D2} & \textbf{D3} & \textbf{D4} & \textbf{D5} & \textbf{D6} \\
\midrule
\endfirsthead

\multicolumn{8}{l}{\emph{(Cuadro~\ref{tab:sota-mapa}, continúa de la página anterior)}}\\
\toprule
\textbf{Sistema} & \textbf{Año} & \textbf{D1} & \textbf{D2} & \textbf{D3} & \textbf{D4} & \textbf{D5} & \textbf{D6} \\
\midrule
\endhead
...
Seq2SQL~\cite{Seq2SQL2017}              & 2017 & TF, TR     & M       & ---     & ---       & D       & C       \\
IRNet/SemQL~\cite{guo2019irnet}         & 2019 & TF         & PM      & LD      & ---       & D       & C, In   \\
BRIDGE~\cite{BRIDGE2020}                & 2020 & TF         & M       & ---     & CR        & D       & C       \\
NatSQL~\cite{gan2021natsql}             & 2021 & TF         & M       & LD      & ---       & D       & C, In   \\
RAT-SQL~\cite{RAT-SQL2021}              & 2021 & TF         & M       & EI      & SL        & D       & C       \\
RaSaP~\cite{RaSaP2021}                  & 2021 & TF         & M       & AB      & ---       & D       & C       \\
SmBoP~\cite{SmBoP2021}                  & 2021 & TF         & M       & AB      & ---       & D       & C       \\
Graphix-T5~\cite{Graphix-T52022}        & 2022 & TF         & M       & EI      & SL        & D       & C       \\
PICARD~\cite{PICARD2022}                & 2022 & TF         & M       & AB      & ---       & D       & C       \\
RASAT~\cite{RASAT2022}                  & 2022 & TF         & M       & ---     & ---       & D       & C       \\
S$^{2}$SQL~\cite{S2SQL2022}             & 2022 & TF         & M       & EI      & SL        & D       & C       \\
SHiP~\cite{SHiP2022}                    & 2022 & TF         & M       & ---     & ---       & D       & C       \\
TKK~\cite{TKK2022}                      & 2022 & TM         & PM      & ---     & SL        & D       & C       \\
ACT-SQL~\cite{ACT-SQL2023}              & 2023 & FF         & M       & ---     & ---       & CoT     & C       \\
C3-SQL~\cite{C3-SQL2023}                & 2023 & F0         & PM      & ---     & ---       & D       & C       \\
CatSQL~\cite{CatSQL2023}                & 2023 & TF         & M       & LD      & ---       & D       & C       \\
CRUSH4SQL~\cite{CRUSH4SQL2023}          & 2023 & FF         & PM      & ---     & SL        & D       & C, Es   \\
DAIL-SQL~\cite{DAIL-SQL2023}            & 2023 & FF         & M       & ---     & ---       & CoT     & C       \\
DIN-SQL~\cite{DIN-SQL2023}              & 2023 & FF         & PM      & DD      & SL, AR    & Dc, It  & C, In   \\
G$^{3}$R~\cite{G3R2023}                 & 2023 & TF         & RR      & ---     & ---       & MC      & C       \\
MAC-SQL~\cite{MAC-SQL2023}              & 2023 & FF         & MA      & DD      & SL, AR    & Dc, It  & C, In   \\
ODIS~\cite{ODIS2023}                    & 2023 & FF         & M       & ---     & ---       & CoT     & C       \\
RESDSQL~\cite{RESDSQL2023}              & 2023 & TF         & PM      & ---     & SL        & D       & C       \\
SelECT-SQL/SC-Prompt~\cite{SC-Prompt2023} & 2023 & FF       & PM      & ---     & ---       & D       & C       \\
SQLFormer~\cite{SQLFormer2023}          & 2023 & TF         & M       & PE, AB  & ---       & D       & C       \\
T5-3B+NatSQL~\cite{T5-3B+NatSQL+TokenPreprocessing2023} & 2023 & TF & M & LD & ---  & D       & C       \\
CHESS~\cite{CHESS2024}                  & 2024 & FF         & MA      & DD      & SL, CR, AR & Dc, It & C, In   \\
CodeS~\cite{CodeS2024}                  & 2024 & TC         & M       & ---     & ---       & D       & C, Es   \\
CoE-SQL~\cite{CoE-SQL2024}              & 2024 & FF         & PM      & ---     & ---       & CoT     & C       \\
DEA-SQL~\cite{DEA-SQL2024}              & 2024 & FF         & PM      & ---     & SL, AR    & Dc, It  & C       \\
DTS-SQL~\cite{DTS-SQL2024}              & 2024 & TF         & PM      & ---     & SL        & D       & C       \\
FinSQL~\cite{FinSQL2024}                & 2024 & TF         & PM      & ---     & CR        & D       & C, Ef, Ad \\
MetaSQL~\cite{MetaSQL2024}              & 2024 & H          & RR      & ---     & ---       & MC      & C       \\
PET-SQL~\cite{PET-SQL2024}              & 2024 & FF         & PM      & ---     & CR        & CoT     & C, Ef   \\
PURPLE~\cite{PURPLE2024}                & 2024 & FF         & PM      & ---     & SL, AR    & It      & C       \\
RSL-SQL~\cite{RSL-SQL2024}              & 2024 & FF         & PM      & ---     & SL, AR    & Dc, It  & C       \\
SuperSQL~\cite{SuperSQL2024}            & 2024 & FF         & PM      & ---     & CR        & CoT, Dc & C, Ef   \\
TA-SQL~\cite{TA-SQL2024}                & 2024 & FF         & PM      & ---     & SL        & Dc      & C       \\
XiYan-SQL~\cite{XiYan-SQL2024}          & 2024 & H          & MA, RR  & EI      & CR, AR    & MC, It  & C       \\
ZeroNL2SQL~\cite{ZeroNL2SQL2024}        & 2024 & H          & PM      & LD      & ---       & D       & C, Ad   \\
Alpha-SQL~\cite{Alpha-SQL2025}          & 2025 & F0         & AS      & ---     & AR        & It      & C       \\
BLENDSQL~\cite{BLENDSQL2025}            & 2025 & FF         & PM      & ---     & ---       & D       & C       \\
CHASE-SQL~\cite{CHASE-SQL2025}          & 2025 & H          & MA, RR, AS & ---  & CR, AR    & MC, It  & C, Es   \\
CRED-SQL~\cite{CRED-SQL2025}            & 2025 & FF, FD     & PM      & ---     & CR        & CoT     & C, Ad   \\
Gen-SQL~\cite{Gen-SQL2025}              & 2025 & FF         & PM      & ---     & ---       & D       & C       \\
OpenSearch-SQL~\cite{OpenSearch-SQL2025} & 2025 & FD        & MA      & ---     & CR, AR    & It      & C       \\
PARSQL~\cite{PARSQL2025}                & 2025 & TM         & RR      & ---     & ---       & MC      & C       \\
ROUTE~\cite{ROUTE2025}                  & 2025 & TM         & PM      & ---     & EK        & D       & R       \\
SGU-SQL~\cite{SGU-SQL2025}              & 2025 & FF         & M       & ---     & SL        & D       & C       \\
SQLHD~\cite{SQLHD2025}                  & 2025 & ---        & ---     & ---     & AR        & It      & R       \\
STRuCT-LLM~\cite{STRuCT-LLM2025}        & 2025 & TR         & M       & ---     & ---       & D       & C       \\
SteinerSQL~\cite{SteinerSQL2025}        & 2025 & FF         & PM      & PE      & SL        & Dc      & C       \\
TS-SQL~\cite{TS-SQL2025}                & 2025 & FF         & MA      & ---     & SL        & Dc      & C       \\
Wren AI~\cite{WrenAI2025}               & 2025 & FF         & PM      & ---     & ---       & D, It   & C       \\
ErrorLLM~\cite{errorllm2026}            & 2026 & ---        & ---     & ---     & AR        & It      & R       \\

\end{longtable}
}
\renewcommand{\arraystretch}{1.0}

El cuadro hace evidentes tres patrones que la discusión en prosa solo describe parcialmente. Primero, la concentración de los trabajos recientes (2024--2026) en arquitecturas modulares y multiagente con razonamiento descompositivo o iterativo, en contraste con el predominio de configuraciones monolíticas y razonamiento directo en el período 2017--2022. Segundo, la persistencia de la columna D3 con el valor \texttt{---} en la mayoría de los sistemas posteriores a 2022, evidencia de que la representación intermedia explícita ha sido una opción minoritaria frente a la decodificación libre. Tercero, la columna D6 muestra una concentración casi exclusiva en \emph{C} (corrección), con una presencia muy puntual de las propiedades restantes; el contraste con las exigencias del entorno empresarial es uno de los argumentos centrales que articula la propuesta del Capítulo~\ref{chap:proposal}. % LLM-like: cliché-LLM:"articula"
