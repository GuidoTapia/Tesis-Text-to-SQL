...
Los \acp{LLM} aplicados a \emph{text-to-query} tienden a producir consultas \ac{SQL}/\ac{PGQ} plausibles pero incorrectas o no fundamentadas, debido a fallas de anclaje al esquema, ambigüedad semántica y ausencia de restricciones durante la generación; en entornos empresariales esto se traduce en \emph{alucinaciones} que comprometen la corrección, la seguridad y el rendimiento, especialmente en el estándar híbrido \ac{SQL}/\ac{PGQ}. Por tanto, el problema que aborda esta tesis es: \emph{¿cómo diseñar e implementar una arquitectura de razonamiento estructural basada en \ac{IR} que mitigue sistemáticamente las alucinaciones en la generación \emph{text-to-query} y produzca consultas \ac{SQL}/\ac{PGQ} correctas, verificables y operables bajo restricciones empresariales


\section{Objetivos}

\textbf{Objetivo general 1:} Diseñar una arquitectura para la generación \emph{text-to-query} orientada al estandar \ac{SQL}/\ac{PGQ} basada en razonamiento estructural y una \ac{IR} tipada, capaz de reducir alucinaciones de esquema y errores semánticos.
...
\begin{itemize}
    ...
    \item Diseñar una \ac{IR} unificada para consultas híbridas que represente (i) componentes relacionales (proyección, selección, \emph{joins}, agregación) y (ii) componentes de patrón de grafo (variables, aristas, restricciones), junto con reglas de tipado, canonicalización y compilación determinista a \ac{SQL}/\ac{PGQ} conforme al estándar \cite{iso9075_16_2023}.
    \item Construir un \emph{pipeline} de razonamiento estructural que integre: anclaje al esquema, generación restringida (p.\,ej., decodificación guiada por gramática o validación incremental en el espíritu de \cite{picard2021}), y un bucle de verificación que combine chequeos estáticos y ejecución controlada (sandbox) con criterios de costo/seguridad.
    \item Definir un protocolo experimental y un conjunto de evaluación que cubra \emph{text-to-SQL} y \emph{text-to-graph queries} (incluyendo benchmarks enterprise y de grafos), y evaluar la propuesta frente a métodos comparables (p.\,ej., \cite{din_sql_2023,tasql2024,errorllm2026,sqlhd2025}) usando métricas de exactitud por ejecución, tasa de alucinación de esquema, robustez a drift y sobrecosto de latencia.
\end{itemize}

\section{Organización de la tesis}
El restante de este documento de tesis está organizado de la siguiente forma:
\begin{itemize}
	\item El capítulo~\ref{chap:background} describe el marco teórico y...
	\item El Capítulo~\ref{chap:proposal} presenta ...
\end{itemize}

\begin{itemize}
	\item El capítulo~\ref{chap:background} describe el marco teórico, para un mejor entendimiento del alcance de \acp{LLM} y alucinaciones, fundamentos de \emph{text-to-SQL}, bases de \ac{SQL}/\ac{PGQ}, y técnicas de \ac{IR}, decodificación restringida y verificación.
	...
	\item El capítulo~\ref{chap:proposal} describe la propuesta: la \ac{IR} tipada para \ac{SQL}/\ac{PGQ}, el compilador \ac{IR}$\rightarrow$\ac{SQL}/\ac{PGQ}, el pipeline de razonamiento estructural y el bucle de verificación (estático y por ejecución).
